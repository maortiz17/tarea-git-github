...
public class ProductoDAOOracle implements IProductoDAO {
	...
	public Producto crear(Connection con, String barcode, String nombre, double precio) throws SQLException {
        ...
        String[] pk = {"id"};
        // En la llamada al mÈtodo prepareStatement pasamos un Array con los campos que componen la PD de la tabla
        // AsÌ tendremos disponible el mÈtodo getGeneratedKeys con la PK generada en el INSERT
    ...
    public Producto buscar(Connection con, long id) throws SQLException{
        ...
        // Devolvemos un objeto Producto cuyo id se recibe por par√°metro
    ...
    public Producto modificar(Connection con, long id, String barcode, String nombre, double precio) throws SQLException {
        ...
        // Actualizamos el producto con el id recibido como par√°metro con el resto de par√°metros
        try(PreparedStatement pstmt = con.prepareStatement(query)){
            pstmt.setString(1, barcode);
            pstmt.setString(2, nombre);
            pstmt.setDouble(3, precio);
    ...
    public boolean borrar(Connection con, long id) throws SQLException {
        ...
        // Eliminamos el producto cuyo id se recibe como par√°metro
        try(PreparedStatement pstmt = con.prepareStatement(query)){
            pstmt.setLong(1, id);
